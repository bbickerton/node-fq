...
FqMessage.prototype.setMsgId = function(val){
    if(this.msgId === null) this.msgId = new Buffer(16);
    if(typeof val === 'string') return this.msgId.write(val,0,16,'utf8');
    if(val instanceof Buffer) val.copy(this.msg,0,0,16);
};

FqMessage.prototype.setRoute = function(val){
    if(typeof val === 'string') val = new Buffer(val, 'utf8');
    this.route = val;
...
FqMessage.prototype.read = function(client, callback){
    var self = this;
    if(self.isComplete()) return callback(null, true);
    async.series({
...
FqMessage.prototype.send = function(client, callback){
    var self = this;
    if(!self.isComplete()) return callback(new Error('Incomplete Message'));
    if(self.iovec === null){
        var i = 0;
        var iovec = [];
